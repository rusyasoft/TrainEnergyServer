...
def wrapSubwayTotalInfo(Trains, bigTable, str_curtime):  # str_curtime must be "hh:mm" format
   ...
   #temporary temp
   temporary_temp_int = 25 + int(time.strftime("%M"))%10
   temporary_hum_int =  40 + int(time.strftime("%M"))%10 # just for distinguishing with temporary_temp_int
   ...
   for sublocdir in schedule_list:
      ...
      if subwayNum in Trains:
         print 'subway is detected', subwayNum
         subw['Area'] = 'Gwangju'
